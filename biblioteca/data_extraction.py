...
def download_one_excel_data(ano, driver):
    if ano != 2022:
        ...
        driver.find_element(by='xpath', value='//*[@id="publicacao"]/div/div[2]/section/div[4]/a').click()
    else:
        # URL da página que contém o botão de download
...
def excel_file_nomination(ano):
    if ano in [2007, 2009, 2010, 2011, 2013]:
        # Especifique o caminho completo do arquivo que você deseja mover
        origin = os.path.join(os.path.expanduser('~'), 'Downloads', f'Fundeb{ano}.xls')
    elif ano == 2008:
        # Especifique o caminho completo do arquivo que você deseja mover
        origin = os.path.join(os.path.expanduser('~'), 'Downloads', f'Fundeb_{ano}.xls')
    elif ano in [2012, 2014, 2015, 2016, 2017]:
        # Especifique o caminho completo do arquivo que você deseja mover
        origin = os.path.join(os.path.expanduser('~'), 'Downloads', f'pge_fundeb_{ano}.xls')
    else:
        # Especifique o caminho completo do arquivo que você deseja mover
        origin = os.path.join(os.path.expanduser('~'), 'Downloads', f'Fundeb {ano}.xls')
    return origin
...
def download_all_excel_datas(destiny_fold):
    ...
    for ano in range(2007, corrent_year+1, 1):
        if f'Fundeb {ano}.xls' not in os.listdir(path=destiny_fold):
            ...
            download_one_excel_data(ano, driver)
            # Especificando pasta de destino padrão - Downloads
            origin = excel_file_nomination(ano=ano)
